...
async function pipeline(_options, cache) {
  ...
  const { taxonomy } = await (0, _gpt.default)(
    options.model,
    options.apiKey,
    "taxonomy",
    (0, _prompts.systemMessage)(options),
    (0, _prompts.clusteringPrompt)(options, comments),
    tracker,
    cache,
  );
  ...
  for (let i = 0; i < options.data.length; i += options.batchSize) {
    ...
    await Promise.all(
      batch.map(async ({ id, comment }) => {
        const { claims } = await (0, _gpt.default)(
          options.model,
          options.apiKey,
          "claims_from_" + id,
          (0, _prompts.systemMessage)(options),
          (0, _prompts.extractionPrompt)(
            options,
  ...
  for (const topic of taxonomy) {
    for (const subtopic of topic.subtopics) {
      const { nesting } = await (0, _gpt.default)(
        options.model,
        options.apiKey,
        "nesting_" +
          subtopic.subtopicName
            ...
            .replace(/\s/g, "_"),
        (0, _prompts.systemMessage)(options),
        (0, _prompts.dedupPrompt)(options, JSON.stringify(subtopic.claims)),
        tracker,
        cache,
      );

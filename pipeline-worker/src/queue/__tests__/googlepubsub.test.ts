...
describe("GooglePubSub", () => {
  ...
  describe("enqueue", () => {
    it("should publish a message successfully", async () => {
      ...
      const testData = { id: "test-id", value: 42 };
    ...
    it("should handle publish errors", async () => {
      mockTopic.publishMessage.mockRejectedValue(new Error("Publish failed"));

      const testData = { id: "test-id", value: 42 };

      await expect(pubsub.enqueue(testData)).rejects.toThrow(
        "Failed to publish message to topic test-topic: Error: Publish failed",
      );
    });
  });

...
describe.skipIf(!dockerAvailable)(
  "Lock Refresh and Expiration Integration Tests",
  () => {
    ...
    describe("Lock Refresh During Long-Running Steps", () => {
      it.skip("should periodically refresh lock during step execution", async () => {
        // NOTE: This test is skipped because LOCK_REFRESH_INTERVAL_MS is 3.5 minutes.
        // A 2-second test step won't trigger any lock refreshes.
        // Lock refresh mechanism is tested indirectly through longer-running integration tests.
      ...
      it("should abort step execution when lock refresh fails", async () => {
        ...
        // Pipeline should detect lock loss and fail
        // It may throw an error or return a failure result
        try {
          const result = await runPipeline(input, config, stateStore);
          expect(result.success).toBe(false);
          if (!result.success) {
          ...
          // Lock loss during save throws an error
          expect(error).toBeInstanceOf(Error);
          expect((error as Error).message).toContain("lock");
        }

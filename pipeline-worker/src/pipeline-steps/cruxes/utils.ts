...
export function calculateTopicScores(
  ...
  for (const topicName in subtopicCruxesByTopic) {
    ...
    const controversyScores = cruxes.map((c) => c.controversyScore);
    const avgControversy =
      controversyScores.reduce((sum, score) => sum + score, 0) /
      controversyScores.length;

    // Count unique speakers across all subtopics
    const allSpeakers = new Set<string>();
    for (const crux of cruxes) {
      for (const s of crux.agree) {
        allSpeakers.add(s);
      }
      for (const s of crux.disagree) {
        allSpeakers.add(s);
      }
    }
    ...
    topicScores.push({
      topic: topicName,
      averageControversy: avgControversy,

...
function processSubtopicResult(
  ...
  // Parse and validate the crux response structure
  if (
    !llmResponse ||
    ...
    reportLogger.error(
      { cruxIdentifier: item.cruxIdentifier, llmResponse },
      "Invalid crux response structure - missing 'crux' property",
    );
    return failure(
      new ParseFailedError(
        item.cruxIdentifier,
        "Invalid response structure: missing 'crux' property",
      ),
    );
  }

  const crux = (llmResponse as { crux: unknown }).crux;

  // Validate crux object has required properties
  if (
    !crux ||
    typeof crux !== "object" ||
    !("cruxClaim" in crux) ||
    ...
    reportLogger.error(
      { cruxIdentifier: item.cruxIdentifier, crux },
      "Invalid crux object - missing required properties",
    );
    return failure(
      new ParseFailedError(
        item.cruxIdentifier,
        "Invalid crux object: missing required properties (cruxClaim, agree, disagree)",
      ),
    );
  }

  // Extract and validate crux properties with proper typing
  const cruxTyped = crux as {
    cruxClaim: unknown;
    agree: unknown;
    disagree: unknown;
    no_clear_position?: unknown;
    explanation?: unknown;
  };

  const cruxClaim =
    typeof cruxTyped.cruxClaim === "string" ? cruxTyped.cruxClaim : "";
  const agree = Array.isArray(cruxTyped.agree) ? cruxTyped.agree : [];
  const disagree = Array.isArray(cruxTyped.disagree) ? cruxTyped.disagree : [];
  const noClearPosition = Array.isArray(cruxTyped.no_clear_position)
    ? cruxTyped.no_clear_position
    : [];
  const explanation =
    typeof cruxTyped.explanation === "string" ? cruxTyped.explanation : "N/A";

...
function parseAndValidateClaims(
  ...
  for (const rawClaim of parsed.claims || []) {
    // Validate that topic and subtopic are from the taxonomy
    if (!validTopicNames.includes(rawClaim.topicName)) {
      claimsLogger.warn(
        { topicName: rawClaim.topicName, validTopics: validTopicNames },
        "Claim has invalid topic name, skipping",
      );
      continue;
    }
    if (!validSubtopicNames.includes(rawClaim.subtopicName)) {
      claimsLogger.warn(
        {
          subtopicName: rawClaim.subtopicName,
          validSubtopics: validSubtopicNames,
        },
        "Claim has invalid subtopic name, skipping",
      );
      continue;
    }
...
function buildTaxonomyPromptSection(taxonomy: Topic[]): string {
  ...
  for (const topic of taxonomy) {
    const topicName = escapeQuotes(topic.topicName);
    const topicDesc = escapeQuotes(topic.topicShortDescription || "");

    if (topicDesc) {
      lines.push(`Topic: "${topicName}" - ${topicDesc}`);
    } else {
    ...
    if (subtopics.length > 0) {
      for (const subtopic of subtopics) {
        const subtopicName = escapeQuotes(subtopic.subtopicName);
        const subtopicDesc = escapeQuotes(
          subtopic.subtopicShortDescription || "",
        );

        if (subtopicDesc) {
          lines.push(`  - Subtopic: "${subtopicName}" - ${subtopicDesc}`);
        } else {
          lines.push(`  - Subtopic: "${subtopicName}"`);
        }
      }
    }
    lines.push("");
...
export async function extractClaimsFromComment(
  ...
  const { validTopicNames, validSubtopicNames } = taxonomyValidation.value;
...
function runClaimsEvaluation(
  ...
  const datasetRow = {
    ...
    taxonomy: taxonomy.map((t) => ({
      ...
      subtopics: t.subtopics.map((s) => s.subtopicName),
    })),
  };

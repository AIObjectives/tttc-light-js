...
function buildClaimsTree(
  ...
  // Build a set of valid topic and subtopic combinations for validation
  const validTopics = new Set<string>();
  const validSubtopics = new Map<string, Set<string>>(); // topic -> set of subtopic names

  // Initialize the tree structure from taxonomy
  ...
  // Populate the tree with claims
  for (const claim of claims) {
    const topicName = claim.topicName;
    const subtopicName = claim.subtopicName;

    // Validate that the claim references a valid topic from the taxonomy
    ...
    // Validate that the claim references a valid subtopic for this topic
      ...
      reportLogger.warn(
        {
          ...
          subtopicName,
          commentId: claim.commentId,
          validSubtopicsForTopic: Array.from(
            validSubtopics.get(topicName) || [],
          ),
        },
    ...
    // Add claim to subtopic (we've already validated it exists)

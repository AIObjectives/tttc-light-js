...
export function basicSanitize(
  ...
  if (text.length > MAX_COMMENT_LENGTH) {
    console.warn(`Oversized input in ${context}: ${text.length} chars`);
  ...
  for (const pattern of INJECTION_PATTERNS) {
    if (pattern.test(sanitizedText)) {
      console.warn(`Potential prompt injection in ${context}: ${pattern}`);
      return { sanitizedText: "", isSafe: false };
    }
  }
...
export function sanitizePromptLength(prompt: string): string {
  if (prompt.length > MAX_PROMPT_LENGTH) {
    console.warn(`Truncating oversized prompt: ${prompt.length} chars`);

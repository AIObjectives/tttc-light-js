...
export async function commentsToTree(
  ...
  try {
    clusteringResult = await callClusteringModel(
      client,
    ...
    reportLogger.error("Failed to call clustering model", error);
    return {
      data: [],
  ...
  reportLogger.info(
    `Topic tree generated: ${clusteringResult.taxonomy.length} topics, ` +
      `${clusteringResult.usage.total_tokens} tokens, $${clusteringResult.cost.toFixed(4)} cost`,
  );

  const responseData: TopicTreeResult = {
    data: clusteringResult.taxonomy,
    usage: clusteringResult.usage,
    cost: clusteringResult.cost,
  };

...
describe("reconcileCruxSpeakers", () => {
  ...
  describe("Data quality logging", () => {
    ...
    beforeEach(() => {
      // Spy on console.info (used instead of pino to avoid webpack bundling issues)
      consoleInfoSpy = vi.spyOn(console, "info").mockImplementation(() => {});
    });

    afterEach(() => {
      // Restore original console.info after each test
    ...
    it("logs metrics when ambiguous speakers are found", () => {
      ...
      // Verify console.info was called with structured data
      expect(consoleInfoSpy).toHaveBeenCalled();
      const [message, logData] = consoleInfoSpy.mock.calls[0];
      expect(message).toBe(
        "[crux-reconciliation] Crux speaker reconciliation applied",
      );
    ...
    it("logs metrics when duplicates are removed", () => {
      ...
      expect(consoleInfoSpy).toHaveBeenCalled();
      const [, logData] = consoleInfoSpy.mock.calls[0];
    ...
    it("correctly counts empty string filtering", () => {
      ...
      expect(consoleInfoSpy).not.toHaveBeenCalled(); // No deduplication of valid speakers
    });
  });
});

...
import type { AnalyticsConfig, AnalyticsEvent, AnalyticsIdentify, AnalyticsContext } from '../types';
...
describe('PostHogAnalyticsProvider', () => {
  ...
  describe('track', () => {
    ...
    it('should implement retry logic on track failure', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining('track operation failed for test_event, retrying...'),
        expect.any(Error)
      );
    });
  ...
  describe('identify', () => {
    ...
    it('should implement retry logic on identify failure', async () => {
      ...
      const { logger } = await import('../../logger');
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining('identify operation failed for user123, retrying...'),
        expect.any(Error)
      );
    });
  ...
  describe('page', () => {
    ...
    it('should handle page tracking with retry logic', async () => {
      ...
      const { logger } = await import('../../logger');
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining('page operation failed for Test Page, retrying...'),
        expect.any(Error)
      );
    });
  ...
  describe('shutdown', () => {
    ...
    it('should handle shutdown errors gracefully', async () => {
      mockPostHogInstance.shutdown.mockRejectedValue(new Error('Shutdown failed'));

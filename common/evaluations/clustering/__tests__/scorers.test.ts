...
import { sampleComments } from "../datasets.js";
...
describe("Clustering Scorers", () => {
  describe("jsonStructureScorer", () => {
    ...
    it("should reject topic description that is too short or too long", () => {
      ...
      expect(result.valid_json_structure).toBe(false);
      expect(result.error).toContain("Topic description must be 25-30 words");
    });
  ...
  describe("topicCoverageScorer", () => {
    ...
    it("should handle missing taxonomy", () => {
      ...
      expect(result.error).toBe("No taxonomy found");
    });
  });

  describe("Sample Data", () => {
    it("should have valid sample comments", () => {
      expect(sampleComments.toLowerCase()).toContain("cats");
      expect(sampleComments.toLowerCase()).toContain("dogs");
      expect(sampleComments.toLowerCase()).toContain("birds");
    });
  });

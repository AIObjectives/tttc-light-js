    ...
    for (const topic of modelOutput.taxonomy) {
      ...
      for (const subtopic of topic.subtopics) {
        ...
        if (subtopicWordCount < 70 || subtopicWordCount > 90) {
          return {
            valid_json_structure: false,
            error: `Subtopic description must be 70-90 words: got ${subtopicWordCount} words`,
          };
        }
      }
    }
...
export const topicCoverageScorer = weave.op(function topicCoverageScorer({
  ...
  if (!modelOutput?.taxonomy) {
    return { topic_coverage_score: 0, error: "No taxonomy found" };
  }
...
export function createClusteringModel(
  ...
  return weave.op(async function clusteringModel(input: {
    ...
    try {
      ...
      evaluationLogger.error(error, "failed to parson JSON response");
      throw error;
    }
  });
}

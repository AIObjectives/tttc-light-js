...
async function pipeline(_options, cache) {
  ...
  const {
    taxonomy
  } = await (0, _gpt.default)("taxonomy", (0, _prompts.systemMessage)(options), (0, _prompts.clusteringPrompt)(options, comments), tracker, cache);
  ...
  for (let i = 0; i < options.data.length; i += options.batchSize) {
    ...
    await Promise.all(batch.map(async ({
      ...
      const {
        claims
      } = await (0, _gpt.default)("claims_from_" + id, (0, _prompts.systemMessage)(options), (0, _prompts.extractionPrompt)(options, JSON.stringify(taxonomy), comment), tracker, cache);
  ...
  for (const topic of taxonomy) {
    for (const subtopic of topic.subtopics) {
      const {
        nesting
      } = await (0, _gpt.default)("nesting_" + subtopic.subtopicName.replace(/[^a-zA-Z0-9 ]/g, "").replace(/\s/g, "_"), (0, _prompts.systemMessage)(options), (0, _prompts.dedupPrompt)(JSON.stringify(subtopic.claims)), tracker, cache);
...
var _default = exports.default = pipeline; // const styles = `
// body {
//   margin: 20px
// }
// h2, h3 {

...
describe("getControversyColors", () => {
  it("returns red colors for very high controversy (>=0.8)", () => {
    const colors = getControversyColors(0.85);
    expect(colors.bg).toBe("bg-red-100");
    expect(colors.text).toBe("text-red-800");
    expect(colors.border).toBe("border-red-300");
  });

  it("returns orange colors for high controversy (>=0.5 and <0.8)", () => {
    const colors = getControversyColors(0.6);
    expect(colors.bg).toBe("bg-orange-100");
    expect(colors.text).toBe("text-orange-800");
    expect(colors.border).toBe("border-orange-300");
  });

  it("returns yellow colors for moderate controversy (>=0.2 and <0.5)", () => {
  ...
  it("returns green colors for low controversy (<0.2)", () => {
...
describe("getControversyCategory", () => {
  it("categorizes scores correctly at boundaries", () => {
    // Very high: >= 0.8
    expect(getControversyCategory(0.8).level).toBe("veryHigh");
    expect(getControversyCategory(1.0).level).toBe("veryHigh");
    expect(getControversyCategory(0.85).level).toBe("veryHigh");
  ...
  it("includes correct labels and descriptions", () => {
    const veryHigh = getControversyCategory(0.9);
    expect(veryHigh.label).toBe("Very high");
    expect(veryHigh.description).toBe("Strong polarization among participants");

    const high = getControversyCategory(0.6);
    expect(high.label).toBe("High");
    expect(high.description).toBe(
      "Significant disagreement among participants",
    );

    const moderate = getControversyCategory(0.3);

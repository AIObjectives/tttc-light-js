...
/**
 * Controversy score thresholds (as percentages 0-100)
 *
 * These thresholds are designed to create 4 roughly equal-sized buckets
 * that map to our icon system and provide meaningful distinction:
 *
 * - VERY_HIGH (80-100%): Strong polarization - participants are deeply divided
 * - HIGH (50-79%): Significant disagreement - clear opposing viewpoints
 * - MODERATE (20-49%): Some disagreement - mixed opinions but not extreme
 * - LOW (0-19%): General consensus - most participants agree
 *
 * The 80% threshold ensures "very high" is reserved for truly polarizing topics.
 * The 50% midpoint distinguishes topics with majority disagreement vs mixed opinions.
 * The 20% floor ensures we don't overstate minor disagreements as "moderate".
 */
const CONTROVERSY_THRESHOLD_VERY_HIGH = 80;
const CONTROVERSY_THRESHOLD_HIGH = 50;
const CONTROVERSY_THRESHOLD_MODERATE = 20;

// Build-time validation: Ensure thresholds are in descending order
// This will cause a TypeScript compilation error if thresholds are incorrectly ordered
type _ThresholdValidation = typeof CONTROVERSY_THRESHOLD_VERY_HIGH extends 80
  ? typeof CONTROVERSY_THRESHOLD_HIGH extends 50
    ? typeof CONTROVERSY_THRESHOLD_MODERATE extends 20
      ? 80 extends number
        ? 50 extends number
          ? 20 extends number
            ? true
...
// This constant will fail to compile if thresholds are out of order
const _validateThresholds: _ThresholdValidation = true;

/**
 * Get controversy category based on score (0-1 range)
 * Maps to 4 tiers matching the icon system
 *
...
export function getControversyCategory(score: number): ControversyCategory {
  ...
  if (percentage >= CONTROVERSY_THRESHOLD_VERY_HIGH) {
    return {
      level: "veryHigh",
      label: "Very high",
      description: "Strong polarization among participants",
    };
  }
  if (percentage >= CONTROVERSY_THRESHOLD_HIGH) {
    return {
      level: "high",
      label: "High",
  ...
  if (percentage >= CONTROVERSY_THRESHOLD_MODERATE) {
    return {
      level: "moderate",
      label: "Moderate",

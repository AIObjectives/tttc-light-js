...
describe("AgreeDisagreeSpectrum", () => {
  ...
  describe("Deterministic positioning (hash function)", () => {
    ...
    it("creates visual spread within each region", () => {
      ...
      // Note: Only first 10 speakers are displayed due to MAX_DISPLAY_SPEAKERS
  ...
  describe("Hash function properties", () => {
    ...
    it("validates hash quality for displayed speakers (limited to 10)", () => {
      // Even with 200 speakers provided, only first 10 are rendered
      const manyIds = Array.from({ length: 200 }, (_, i) => `${i}:Speaker${i}`);
      ...
      const dots = getAllByTestId("spectrum-dot") as HTMLElement[];
      expect(dots.length).toBe(10); // Only 10 rendered (limited by MAX_DISPLAY_SPEAKERS)

      // Note: Component does not display a "+N more" indicator
      // Additional speakers beyond MAX_DISPLAY_SPEAKERS are not shown

      // Test hash collision detection on the 10 that ARE shown
      const positions = dots.map((dot) => parseFloat(dot.style.left));
      const uniquePositions = new Set(positions);

      // All 10 displayed speakers should have unique positions
  ...
  describe("Display limit (MAX_DISPLAY_SPEAKERS = 10)", () => {
    it("shows all speakers when total count <= 10 per position", () => {
      const { getAllByTestId } = render(
        <AgreeDisagreeSpectrum
          agree={toSpeakers(["1:A", "2:B", "3:C"])}
          disagree={toSpeakers(["4:D", "5:E"])}
        />,
      );

      expect(getAllByTestId("spectrum-dot")).toHaveLength(5);
      // Note: Component does not display a "+N more" indicator
    });

    it("limits display when any position exceeds 10 speakers", () => {
      const manyAgree = Array.from(
        { length: 15 },
        (_, i) => `${i}:Speaker${i}`,
      );
      ...
      // Only 10 speakers shown (limited by MAX_DISPLAY_SPEAKERS)
      expect(getAllByTestId("spectrum-dot")).toHaveLength(10);
      // Note: 5 speakers are not shown (no indicator displayed)
    });

    it("limits display to exactly 10 speakers per position", () => {
      const exactlyTen = Array.from(
        { length: 10 },
        (_, i) => `${i}:Speaker${i}`,
      );
    ...
    it("aggregates limit across all positions", () => {
      const { getAllByTestId } = render(
        <AgreeDisagreeSpectrum
          agree={toSpeakers(Array.from({ length: 12 }, (_, i) => `${i}:A${i}`))}
          disagree={toSpeakers(
            Array.from({ length: 15 }, (_, i) => `${i + 12}:D${i}`),
          )}
          noClearPosition={toSpeakers(
            Array.from({ length: 8 }, (_, i) => `${i + 27}:N${i}`),
          )}
        />,
      );

      // Should show max 10 per position: 10 agree + 10 disagree + 8 no_clear = 28 total
      // (2 agree + 5 disagree hidden, 7 total hidden)
      expect(getAllByTestId("spectrum-dot")).toHaveLength(28);
    });
  });
  ...
  describe("Real-world usage scenarios", () => {
    ...
    it("limits display to 10 speakers per position for large datasets", () => {
      ...
      // Should render only 20 dots (10 agree + 10 disagree) due to MAX_DISPLAY_SPEAKERS
      const dots = getAllByTestId("spectrum-dot");
      expect(dots.length).toBe(20);

      // Note: The component does not currently display a "+N more" indicator
      // Additional speakers beyond MAX_DISPLAY_SPEAKERS are simply not shown
    });
  });

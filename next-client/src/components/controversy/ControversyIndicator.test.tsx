...
describe("ControversyIndicator", () => {
  describe("Rendering with different controversy levels", () => {
    ...
    it("renders high controversy indicator", () => {
      render(<ControversyIndicator score={0.6} />);

      const labels = screen.getAllByText(/high controversy/i);
      expect(labels.length).toBeGreaterThan(0);
    });

    it("renders very high controversy indicator", () => {
      render(<ControversyIndicator score={0.9} />);

      const labels = screen.getAllByText(/very high controversy/i);
      expect(labels.length).toBeGreaterThan(0);
    });
  });
  ...
  describe("Label visibility", () => {
    ...
    it("hides label when showLabel is false", () => {
      render(<ControversyIndicator score={0.5} showLabel={false} />);

      // Icon should still be present, but no text label visible
      const labels = screen.queryAllByText(/high controversy/i);
  ...
  describe("Controversy category mapping", () => {
    it("maps score 0-0.19 to low controversy", () => {
      render(<ControversyIndicator score={0.15} />);
      expect(screen.getAllByText(/low controversy/i).length).toBeGreaterThan(0);
    });

    it("maps score 0.2-0.49 to moderate controversy", () => {
    ...
    it("maps score 0.5-0.79 to high controversy", () => {
      render(<ControversyIndicator score={0.65} />);
      expect(screen.getAllByText(/high controversy/i).length).toBeGreaterThan(
        0,
      );
    });

    it("maps score 0.8-1.0 to very high controversy", () => {
      render(<ControversyIndicator score={0.85} />);
      expect(
        screen.getAllByText(/very high controversy/i).length,
      ).toBeGreaterThan(0);
    });
    ...
    it("handles boundary values correctly", () => {
      ...
      rerender(<ControversyIndicator score={0.5} />);
      expect(screen.getAllByText(/high controversy/i).length).toBeGreaterThan(
        0,
      );
  ...
  describe("Edge cases", () => {
    ...
    it("handles score exactly at threshold boundaries", () => {
      const { rerender } = render(<ControversyIndicator score={0.199} />);
      expect(screen.getAllByText(/low controversy/i).length).toBeGreaterThan(0);
      ...
      rerender(<ControversyIndicator score={0.499} />);
      expect(
        screen.getAllByText(/moderate controversy/i).length,
      ).toBeGreaterThan(0);

...
/**
 * Maximum number of speakers to display per position.
 * If there are more, show "+N more" indicator.
 */
export const MAX_DISPLAY_SPEAKERS = 10;
 ...
 * Limits display to first 10 speakers per position with "+N more" indicator.
 */
export function AgreeDisagreeSpectrum({
  ...
  // Only show first MAX_DISPLAY_SPEAKERS per position
  const dots = useMemo(() => {
    const allDots: {
      x: number;
      key: string;
    ...
    validDisagree.slice(0, MAX_DISPLAY_SPEAKERS).forEach((speaker, index) => {
    ...
    validNoClear.slice(0, MAX_DISPLAY_SPEAKERS).forEach((speaker, index) => {
    ...
    validAgree.slice(0, MAX_DISPLAY_SPEAKERS).forEach((speaker, index) => {
          ...
          <div
            ...
            style={{
              ...
              backgroundColor: topicColor
                ? `color-mix(in srgb, hsl(var(--theme-${topicColor})) 15%, transparent)`
